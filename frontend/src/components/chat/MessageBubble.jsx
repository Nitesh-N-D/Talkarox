...
export default function MessageBubble({ message, isOwn, preferredLanguage }) {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      // silently fail; original text remains visible
    } finally {
  ...
  return (
    <motion.div
      ...
      className={clsx('flex flex-col max-w-[78%] sm:max-w-[65%]', isOwn ? 'self-end items-end' : 'self-start items-start')}
    >
      {message.category && !isOwn && (
        <div className="mb-1"><CategoryBadge category={message.category} /></div>
      )}
      ...
      <div className="flex items-center gap-2 mt-1 px-1">
        ...
        <span className="text-[11px] text-ink-faint font-mono">{format(new Date(message.createdAt), 'h:mm a')}</span>
        {isOwn && (
          message.readAt
            ? <CheckCheck size={13} className="text-brand-400" />
            : message.pending
